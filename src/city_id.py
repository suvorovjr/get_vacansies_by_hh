...
def receive_city_hh(path_hh, user_city):
    ...
    city_id = [city["id"] for area in data_hh for city in area["areas"] if city["name"].lower() in user_city]
    return city_id
...
def receive_city_super_job(path_sj, user_city):
    ...
    city_id = [city["id"] for city in data_sj if city["title"].lower() in user_city]
    return city_id
